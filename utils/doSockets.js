...
const doSockets = () => {
  ...
  publicIO.on("connection", (socket) => {
    ...
    socket.on("join-editor", () => {
      ...
      socket.on("send-changes", ({ data, value, randID }) => {
        socket.broadcast
          .to("public-room")
          .emit("receive-changes", { data, value, sender_id: randID });
      });
    });
  });
  ...
  chatIO.on("connection", (socket) => {
        ...
        getUserIDFromToken(
          ...
          async ({ found, user_id }) => {
            if (found) {
              ...
              const { found: foundNameUser, user: nameUser } = await findUser({
                username: name,
              });
              if (foundNameUser) {
                if (found) {
                  if (nameUser.username === idUser.username)
                    return socket.emit(
                      "error_happened",
                  ...
                  const { found: foundMessages, messages } = await findMessages(
                    { room: roomID }
                  );
                  if (roomID) {
                    if (roomF) {
                      ...
                      socket.join(roomID);
                      chatIO.to(roomID).emit("onlineUsers", {
                        room: roomID,
                      ...
                      if (foundMessages) {
                        socket.emit("load-messages", {
                          ...
                          profileImage: nameUser.profileImageUrl,
                          profileImageYou: idUser.profileImageUrl,
                          notYouID: nameUser._id,
                        });
                      } else {
                        socket.emit("load-messages", {
                          success: false,
                ...
                socket.emit(
                  "error_happened",
                  "The user you want to chat with does not exist."
                );
              }
            } else {
              socket.emit(
    ...
    socket.on("typing", () => {
      ...
      socket.broadcast
        .to(user?.room)
        .emit("typing", `${user?.username} is typing.`);
    });
    ...
    socket.on("message", async ({ msg }) => {
      ...
      chatIO
        .to(user?.room)
        .emit("message", formatMessage(user && user.username, msg, user?.id));
      ...
      if (user) {
        ...
        const room = await Room.findOne({ roomID: user?.room })
          ...
          .select("profileImageUrl username");
    ...
    socket.on(
      "vn",
      async ({ audiob64, room: roomInUse, idUserProfileImage, id, userID }) => {
        imagekit.upload(
          {
            file: audiob64,
          ...
          async function (error, result) {
            ...
            if (error) console.log(error);
              ...
              chatIO
                ...
                .emit("vn", {
                  body: result.url,
                  user: { profileImageUrl: idUserProfileImage, _id: userID },
                  type: "vn",
                });
              await Message.create({
                user: userID,
            ...
            const room = await Room.findOne({ roomID: roomInUse })
              .populate("participants.user")
              .select("profileImageUrl username");
            const participants = room?.participants;
            ...
            await createNotification(
              notUser?.user?._id,
              `New voice message from ${socket?.currentUser?.username}.`,
              userID
            );
          }
        );
          ...
          async function (error, result) {
            if (error) console.log(error);
              ...
              const room = await Room.findOne({ roomID: roomInUse })
                ...
                .select("profileImageUrl username");
              const participants = room?.participants;
              const notUser = participants.filter(
                (u) => u.user._id != userID
              )[0];
              const payload = JSON.stringify({
  ...
  function userLeavePub(id) {
    const index = ONLINE_PUBLIC_CODE_USERS.findIndex(
  ...
  editorIO.on("connection", (socket) => {
    socket.on("join-editor", async ({ id, pubID, type }) => {
      switch (type) {
        case "normal":
          const { code: normalCode, foundTheCode: foundTheNormalCode } =
            await findCode(id);
          if (foundTheNormalCode) {
            ...
            collabCoderJoin(
              normalCode.user._id,
              "Admin",
              normalCode?.collabLink ?? "",
              socket.id
            );
            socket.emit(
              "load-code",
              {
                language: normalCode.language,
                username: normalCode.user.username,
                name: normalCode.name,
                data: normalCode.data,
              } ?? ""
            );
          } else {
          ...
          socket.on("send-changes", ({ data, value, randID }) => {
            socket.broadcast
              .to(normalCode.publicLink)
              .to(normalCode.collabLink)
              .emit("receive-changes", { data, value, sender_id: randID });
          });
          ...
          break;
        case "public":
          const { code: publicCode, foundTheCode: foundThePublicCode } =
            await findCodeDoc(pubID);
          if (foundThePublicCode) {
            pubCoderJoin(
              "notadmin",
              `user${Math.random().toString().split(".")[1]}`,
              publicCode.publicLink,
              socket.id
            );
            ...
            socket.emit(
              "load-code",
              {
                language: publicCode.language,
                username: publicCode.user.username,
                name: publicCode.name,
                data: publicCode.data,
              } ?? ""
            );
          } else {
            socket.to(socket.id).emit("code_does_not_exist");
        ...
        case "collab":
          ...
          const { code: collabCode, foundTheCode: foundTheCollabCode } =
            await findCodeCollab(pubID);
          if (foundTheCollabCode) {
            pubCoderJoin(
              "notadmin",
              `user${userColID}`,
              collabCode.publicLink,
              socket.id
            );
            ...
            socket.emit(
              "load-code",
              {
                ...
                username: collabCode.user.username,
                name: collabCode.name,
                data: collabCode.data,
              } ?? ""
            );
          } else {
            socket.to(socket.id).emit("code_does_not_exist");
          ...
          socket.on("send-changes", ({ data, value, randID }) => {
            socket.broadcast
              .to(collabCode.publicLink)
              .to(collabCode.collabLink)
              .emit("receive-changes", { data, value, sender_id: randID });
          });
  ...
  async function findCode(id) {
    ...
    return {
      code: "You cannot connect to anyone until you create an account",
      foundTheCode: false,
    };
  }
...
module.exports = { server, app, doSockets, express, feedIO };